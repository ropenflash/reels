...
export default function JumpRopePage() {
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/videos/db?category=rope jumping`);
  ...
  return (
    <div className="flex h-screen flex-col md:flex-row">
      <main className="flex-1 bg-gray-100 p-4 md:p-6 overflow-y-auto">
        <h1 className="text-3xl font-bold mb-4">Jump Rope Videos</h1>
        ...
        {!loading && videos.length === 0 && (
          <p className="text-center text-gray-500 mt-8">No jump rope videos found.</p>
        )}
